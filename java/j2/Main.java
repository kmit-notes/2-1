...
public class Main {
    public static void main(String[] args) throws Exception {
        System.out.println("Travel Platform Booking Intelligence System");
        FileReader f = new FileReader("travel_bookings.csv");
...
System.out.println("Cost");
        ...
        for(String s : hm1.keySet()){
            fw.write(s +"," + (hm2.get(s) / hm1.get(s)) + "\n");
        }
        fw.close();

        File f2 = new File("escalated_bookings.csv");
        FileWriter fw2 = new FileWriter(f2);
        
        for(Booking bi : bl){
            if(bi.category.equals("ESCALATED")){
                fw2.write(bi.toString() + "\n");
            }
        }
        ...
        File f3 = new File("./booking_summary.csv");
        FileInputStream fis = new FileInputStream(f3);
        ZipOutputStream zos = new ZipOutputStream(new FileOutputStream("BookingSummary.zip"));
        ZipEntry ze = new ZipEntry("BookingSummary.csv");
        zos.putNextEntry(ze);
